...
a.  Use the equation above to show the expected change in the odds of maxing out a credit card when the credit rating increases by 10 points. Assume income and region are constant. Write your answer in terms of $\beta_0, \beta_1, \beta_2$, etc.
...
We consider adding the interaction between `region` and `income` to the current model that includes `income`, `rating`, and `region`. We'll use a drop-in-deviance test to determine whether or not to add the interaction term.
...
a.  Construct the ROC curve and compute the area under the curve.
