...
\
There is no solution guide for these problems. You may ask questions in office hours and on Ed Discussion.
:::
...
-   Show mathematically why the slope for $x_j$ and intercept are interpreted in terms of $y$ has shown on [this slide](https://sta221-fa24.netlify.app/slides/16-variable-transformations#/model-interpretation).
...
Recall that for the linear regression, the variance of the estimated coefficients are the diagonal elements of $Var(\hat{\boldsymbol{\beta}}) = \hat{\sigma}^2_{\epsilon}(\mathbf{X}^T\mathbf{X})^{-1}$. One of the [effects of multicollinearity](https://sta221-fa24.netlify.app/slides/15-multicollinearity#/effects-of-multicollinearity) is large variances in the model coefficients. Explain why.

## Exercise 10

Suppose you fit a simple linear regression model and check the model diagnostics.

-   Draw an example of an observation that has high leverage but low Cook's distance.

-   Draw an example of an observation that has high leverage and high Cook's distance.

-   Draw an example of an observation with a large studentized residual.
...
-   What is an advantaged of using a Nested F test instead of AIC (or BIC) to compare models?

-   What is an advantage of using AIC (or BIC) instead of a Nested F test to compare models?
...
-   Show mathematically why the interpretation for the slope for `age` in terms of the log-odds is the form shown on [this slide](https://sta221-fa24.netlify.app/slides/19-logistic-regression#/interpretation-in-terms-of-log-odds-1).

-   Show mathematically why the interpretation for the slope of `age`in terms of the odds is the form shown on [this slide](https://sta221-fa24.netlify.app/slides/19-logistic-regression#/interpretation-in-terms-of-odds-1).

## Exercise 16

Recall the model using age and education to predict odds of being high risk for heart disease.

-   Show mathematically why the interpretation for the slope for `education4` in terms of the log-odds is the form shown on [this slide](https://sta221-fa24.netlify.app/slides/19-logistic-regression#/interpretation-in-terms-of-log-odds).

-   Show mathematically why the interpretation for the slope of `education4`in terms of the odds is the form shown on [this slide](https://sta221-fa24.netlify.app/slides/19-logistic-regression#/interpretation-in-terms-of-odds).
...
-   Draw an example of an ROC curve such that AUC is about 0.55

-   Draw an example of an ROC curve such that AUC is about 0.9.

-   Explain what each point on an ROC curve means.
