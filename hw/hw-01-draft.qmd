...
Let $\mathbf{x}$ is a $k \times 1$ vector and $\mathbf{A}$ is a $k \times k$ matrix, such that $\mathbf{A}$ is not a function of $\mathbf{x}$.
...
In class we used the sum of squared residuals (SSR) to find an estimate of the coefficients $\hat{\boldsymbol{\beta}} = (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{Y}$ . To show this is the least squares estimate, we now need to show that we have, in fact, found the estimate of $\boldsymbol{\beta}$ that minimizes the SSR (rather than maximize).

If the Hessian matrix $\nabla_{\boldsymbol{\beta}}^2 SSR$ is positive definite, then we know we have found the $\hat{\boldsymbol{\beta}}$ that minimizes SSR, i.e., the least squares estimator. Additionally, we have the following:
...
-   `air_temp_avg`: yearly average air temperature in Madison, WI
...
c.  There are separate measurements for each lake in the `icecover` data frame. In this analysis, we will combine the data from both lakes and use the average ice duration each year for the analysis.
