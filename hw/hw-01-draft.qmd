...
You will use simple linear regression to analyze the relationship between air temperature and ice duration for two lakes in Wisconsin. You will also show some of the math behind these models.
...
-   Clone the repo and start a new project in RStudio. See the [Lab 01 instructions](https://sta221-fa24.netlify.app/labs/lab-01#clone-the-repo-start-new-rstudio-project) for details on cloning a repo, starting a new project in R, and configuring git.
...
\
You may write the answers for conceptual exercises by hand or type them in your Quarto document.
:::
...
Given $\mathbf{x}$ is a $k \times 1$ vector and $\mathbf{A}$ is a $k \times k$ matrix, such that $\mathbf{A}$ is not a function of $\mathbf{x}$, show
...
Expalin how we know that this is a minimum.

What would this mean about the predictors in multiple linear regression?

## Exercise 4

Prove that the maximum value of $R^2$ must be less than 1 if the data contains observations such that there are different observed values of the response for the same value of the predictor (e.g., observations $(x_i, y_i)$ and $(x_j, y_j)$ are such that $x_i = x_j$ and $y_i \neq y_j$ ).
...
-   `ice_duration`: a number denoting the number of days between the freeze and ice breakup dates of each lake

-   `ave_air_temp_adjusted`: a number denoting the air temperature in degrees Celsius, collected in Madison, WI and adjusted for biases <!--# update this variable name and definition-->

::: {.callout-note icon="false"}
## Analysis goal

The goal of this analysis is to use linear regression explain variabiltiy in ice duration for lakes in Madison, Wisconsin based on air temperature. Because ice cover is impacted by various environmental factors, researchers are interested in examining the association between these two factors to better understand how quickly the climate is changing.
:::
...
a.  Create side-by-side boxplots to visualize the distribution of `ice_duration` for each lake. <!--# add link to reference on side-by-side boxplots-->

b.  Visualize the distribution of ice duration over time for each lake. <!--# add resource on line plot-->

c.  The current data is currently structured such there are separate measurements for each lake in a given year. We will combine the data such that there is one row per year that contains the average ice duration across the two lakes.

Evaluate the analysis decision to combine the data in this way.

<!--# Need to reword this question-->

## Exercise 6

Next, let's combine the ice duration and air temperature data into a single analysis data frame.

a.  Fill in the code below to create a new data frame, `icecover_avg`, of the average ice duration by lake and year.
...
Then join `icecover_avg` and `airtemp` to create a new data frame. The new data frame should have 268 <!--# check this number--> observations.

b.  Visualize the relationship between the air temperature and average ice duration. Do you think a linear model would be a good fit to capture the relationship between the two variables?
...
a.  State the dimensions of $\mathbf{y}$, $\mathbf{X}$, $\boldsymbol{\beta}$, $\boldsymbol{\epsilon}$ for this analysis. Your answer should have exact value given this data set.
