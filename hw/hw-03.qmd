...
Suppose we the observed value of the predictor `Bush2000` for this county (a new observation) is $x_0$. We define $\mathbf{x}_0^T = [1, x_0]$
...
Just as there is uncertainty in our model coefficients, there is uncertainty in our predictions as well. We use a confidence interval to quantify the uncertainty for a model coefficient, and we can use **prediction intervals** to quantify the uncertainty for the prediction for a new observation.
...
where $t^*_{n-p-1}$ is the critical value obtained from the $t$ distribution with $n - p - 1$ degrees of freedom.

a.  Use the model you chose in the previous question to compute $\hat{y}_0$, the predicted number of votes for Buchanan in the outlying county identified in Exercise 7.

b.  Compute the 95% prediction interval for this county.
...
c.  It is assumed that some of the votes for Buchanan in that county were actually intended to be for Gore. Based on your results in the previous question, does your model support this claim?
    ...
    -   If yes, about how many votes were possibly intended for Gore? Show any calculations and output used to determine your answer.
