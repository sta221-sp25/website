...
When we have a quantitative response, $Y$, and a single quantitative predictor, $X$, we can use a **simple linear regression** model to describe the relationship between $Y$ and $X$. $$\large{Y = \beta_0 + \beta_1X + \epsilon, \hspace{8mm} \epsilon \sim N(0, \sigma_{\epsilon}^2)}$$
...
::: aside
[Click here](https://introregression.netlify.app/98-appendix#sec-least-sq-math) for full details and finding estimate of $\hat{\beta}_1$ .
:::
