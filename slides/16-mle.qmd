...
-   HW 03 March 20 at 11:59pm

-   Project exploratory data analysis due March 20
...
::: incremental
-   How would you describe in words the probabilities we previously calculated? <!--# Not sure what we're getting at. Maybe just say what the question is-->
...
-   **Probability function**: Fixed parameter value(s) + input possible outcomes 
    ...
    -   *Given* $p=0.8$ , *what is the probability of observing* $BBM$ *in three free throws?*
...
## Likelihood: shooting 3 free throws
...
-   MLEs have nice statistical properties (more on this in a future class). They are
...
& = \prod_{i=1}^n p(y_i | x_i, \beta_0, \beta_1, \sigma^2_{\epsilon}) \\[5pt]
...
& = \log\Big[(2\pi\sigma^2_{\epsilon})^{-\frac{n}{2}}\exp\Big\{-\frac{1}{2\sigma^2_{\epsilon}}\sum_{i=1}^n(y_i - \beta_0 - \beta_1x_i)^2\Big\}\Big] \\[8pt]
...
$$
\begin{aligned}
&\Rightarrow \sum_{i=1}^ny_i - n\tilde{\beta}_0 - \tilde{\beta}_1\sum_{i=1}^n x_i = 0 \\[8pt]
&\Rightarrow \sum_{i=1}^ny_i  - \tilde{\beta}_1\sum_{i=1}^n x_i = n\tilde{\beta}_0 \\[8pt]
...
-   MLEs have nice properites, so this means the least-squares estimator $\hat{\mathbf{\boldsymbol{\beta}}}$ inherits all the nice properties of MLEs
