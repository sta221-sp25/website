...
Suppose we want to predict the amount of sleep a Duke student gets based on whether they are in Pratt (Prat Yes/ No are the only two options). Consider the model
...
-   Demonstrate that the design matrix is not of full column rank
    (that is, affirmatively provide one of the columns in terms of
    the others).

-   Use this intuition to explain why when we include categorical
    predictors, we cannot include both indicators for every level of
    the variable *and* an intercept.
:::
...
## Create indicator variables for `verified_income`
