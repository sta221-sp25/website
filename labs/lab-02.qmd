...
In this lab, you will use multiple linear regression to use county-level demographic and labor market factors to predict the median childcare cost for school-aged children. You will analyze data and explain how some concepts from regression theory apply to this analysis. You will also start working with your lab teams, write a team agreement, and be introduced to using GitHub for collaboration.
...
-   use testing data to assess model performance.
-   collaborate with others using a single Github repo.

# **Meet your team!**

[**Click here**](https://prodduke-my.sharepoint.com/:x:/g/personal/mt324_duke_edu/Ea9WHPhr91VAphXfELUbLBsB50LvI-KohMqfQcnpR_6guA?e=JgEpVq) to see the team assignments for STA 221. This will be your team for labs and the final project. Before you get started on the lab, complete the following:
...
-   Go to the [sta221-fa24](https://github.com/sta221-fa24) organization on GitHub. Click on the repo with the prefix **lab-02**. It contains the starter documents you need to complete the lab.
...
**Goal:** The goal of this analysis is to use various demographic and labor-market factors to predict the median price of childcare for school-aged children for counties in North Carolina.
...
As you can see from the data dictionary in the `README` of the `data` folder, there are many interesting potential variables that could be included in the model to predict median childcare cost for school-aged children. Therefore, we will do some *feature selection* and *feature design* to choose potential predictors and construct new ones.
...
-   Transform the variable or use it to create a new variable.
...
Represent the regression model using the matrix representation. For each symbol in the model (1) describe what it represents and (2) state the dimensions. The description and dimensions should be in the context of these data, not in general.

## Exercise 6

Use `lm` to fit the regression model you described in the previous exercise. Neatly display the model using a reasonable number of digits.

-   Interpret the coefficients for each predictor in the context of the data.

-   Based on your model, describe the counties represented by the intercept.
...
-   How much of the variation in the childcare costs is explained by your chosen predictor variables?

```{=html}
<!-- -->
```
-   Based on this, do you think the model explains a significant portion of the variability in childcare costs for school-aged children in North Carolina? Briefly explain.

## Exercise 8

Now let's use the testing data to explore the predictive power of the model.

Use the `predict` function to compute the predicted childcare costs for the observations in the testing data.

<!--# may need to add something about how to store the data-->
...
-   What does this suggest about the predictive power of the model?
